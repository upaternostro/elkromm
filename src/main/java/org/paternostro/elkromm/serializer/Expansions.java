...
public class Expansions implements ElkrommSerializer<Expansion[]>
{
    ...
    public byte[] serialize(Expansion[] obj)
    {
        ...
        for (int i = 0; i < obj.length; i++) {
            ...
            for (int j = 0; j < obj[i].getInputNum(); j++) {
                offset = i * EXPANSION_PAYLOAD_SIZE + j * org.paternostro.elkromm.serializer.Input.PAYLOAD_SIZE + 7;
            ...
            for (int j = 0; j < obj[i].getOutputNum(); j++) {
                offset = i * EXPANSION_PAYLOAD_SIZE + 8 * org.paternostro.elkromm.serializer.Input.PAYLOAD_SIZE + j * org.paternostro.elkromm.serializer.Output.PAYLOAD_SIZE + 7;
    ...
    public Expansion[] deserialize(byte[] data)
    {
         ...
         */
        int patchOffset = 0;
        while ((patchOffset += EXPANSION_PAYLOAD_SIZE - 2) < data.length) data[patchOffset++] = data[patchOffset++] = 0x00;
        // clear not-checksummed excluded bit from inputs
        patchOffset = 0;
        while (patchOffset < data.length - ElkrommUtils.CHECKSUM_SIZE) {
            for (int j = 0; j < 8; j++) {
                data[patchOffset + 7 + j * org.paternostro.elkromm.serializer.Input.PAYLOAD_SIZE + 3] &= ~0x10;
            }
